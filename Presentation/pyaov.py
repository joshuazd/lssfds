...
print "pldat, plper, prew, pspw, totals"
...
def normalize(x,error,mean=0.,var=1.):
    ...
    try:
        ...
        return 0
    try:
        assert x.size == error.size
    except AssertionError:
        print 'Input arrays length must be equal'
        return 0
    try:
        assert var >= 0.
    except AssertionError:
    ...
    try:
        ...
        av = np.sum(x*w)/sw
        y = x-av
        va = np.sum(np.square(y)*w)/sw*n/(n-1)
        if (var>0. and va>0.):
          y=y*np.sqrt(var/va)
        y=y+mean     
        return y
...
def fouw(time, valin, error, frin, bacgnd=0., nh2=2):
    '''
    ...
    Optional input:
        bacgnd[=1.]: (median) AOV periodogram background value==correlation length
               put 0. for (possibly underestimated) LSQ frequency error
    ...
    # maybe something else can go wrong?
    try:
	frout,dfrout,valout,cof,dcof = _aov.aov.fouw(time, valin, error, \
                           frin, bacgnd, nh2)
...
def prew(time, valin, error, frin, bacgnd=0., nh2=2):
    '''
    frout,dfrout,valout=pyaov.prew(time, valin, error, frin bacgnd=0., nh2=2)
    ...
    Input:
        ...
        frin: initial frequency, float
    Optional input:
        bacgnd[=1.]: (median) background value of the nearby AOV periodogram
               put 0. for (possibly overestimated) HWHI frequency error
        nh2[=2]: no. of model parms. (number of harmonics=nh2/2)
...
def amhw(time, amplitude, error, fstop, fstep, fr0=0., nh2=3):
    '''
    th,fr,frmax=pyaov.amhw(time, valin, error, fstop, fstep, fr0=0., nh2=3)
    ...
    # maybe something else can go wrong?
    try:
        th,frmax = _aov.aov.aovmhw(time, amplitude, error, \
            fstep, (fstop-fr0)/fstep+1,fr0=fr0,nh2=nh2)
...
def pspw(time, amplitude, error, fstop, fstep, fr0=0.):
    ...
    # maybe something else can go wrong?
    try:
        th,frmax = _aov.aov.powspw(time, amplitude, error, \
                       fstep, (fstop-fr0)/fstep+1,fr0=fr0)
...
def atrw(time, amplitude, error, fstop, fstep, fr0=0., nh2=30, ncov=2):
    '''
    th,fr,frmax=pyaov.atrw(time, amplitude, error, fstop, fstep, \
         fr0=0., nh2=30, ncov=2)
    ...
    # maybe something else can go wrong?
    try:
        n = int((fstop-fr0)/fstep)+1
        th,frmax = _aov.aov.aovtrw(time, amplitude, error, \
                    fstep, n, fr0=fr0, nh2=nh2, ncov=ncov)
        
        # make an array that contains the frequencies too
        freqs = np.linspace(fr0, fstop, n)
        return th, freqs,frmax
...
def aovw(time, amplitude, error, fstop, fstep, fr0=0., nh2=3, ncov=2):
    '''
    th,fr,frmax=pyaov.aovw(time, valin, error, fstop, fstep, fr0=0., \
       nh2=3, ncov=2)
    ...
    # maybe something else can go wrong?
    try:
        th,frmax = _aov.aov.aovw(time, amplitude, error, fstep, \
                    (fstop-fr0)/fstep+1, nh2=nh2, fr0=fr0, ncov=ncov)
